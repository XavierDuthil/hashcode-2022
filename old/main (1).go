...
func parseFile(exampleFile string) {
	...
	// Parse first line
	scanner.Scan()
	fmt.Printf("%s: %s clients to satisfy\n\n", exampleFile, scanner.Text())
...
OUTER:
	for ingredientLiked, _ := range ingredientsLiked {
		for ingredientDisliked, _ := range ingredientsDisliked {
			if ingredientsDisliked[ingredientDisliked] < ingredientsLiked[ingredientDisliked] {
				break
			}
